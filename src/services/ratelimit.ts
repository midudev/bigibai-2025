...
export function getRateLimitMessage(
  ...
  if (time.minutes < 1) {
    return (
      customMessage ??
      `Demasiados intentos. Intenta de nuevo en ${time.seconds} segundos`
    );
  }
  if (time.minutes < 60) {
    return (
      customMessage ??
      `Demasiados intentos. Intenta de nuevo en ${time.minutes} minutos`
    );
  }
  return (
    customMessage ??
    `Demasiados intentos. Intenta de nuevo en ${time.hours} horas`
  );
}
